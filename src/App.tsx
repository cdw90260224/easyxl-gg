...
import { useAppStore, type Sheet } from './store/useAppStore';
...
export default function App() {
    ...
    // Zustand Store
    const { 
        sheets, setSheets, activeSheetIndex, setActiveSheetIndex, 
        analysisHistory: guestHistory, addHistoryItem: addGuestHistory,
        clearGuestData, isDark, toggleDark, renameSheet 
    } = useAppStore();
    ...
    // Multi-Sheet State (Zustand Store로 완전히 이전됨)
    ...
    // ── LocalStorage 연동 (Zustand Persist로 대체됨) ──

    useEffect(() => {
        if (!supabase) return;

        const migrateGuestData = async (_userId: string) => {
            const { sheets: localSheets, analysisHistory: localHistory } = useAppStore.getState();
            if (localSheets.length === 0 && localHistory.length === 0) return;

            toast.loading("기존 작업 내역을 계정으로 통합하는 중...", { id: 'migration' });
            try {
                // 1. 시트 마이그레이션
                for (const s of localSheets) {
                    await saveSheet(s.name, s.columns.map(c => ({ field: c, headerName: c })), s.data, 'guest_migration');
                }
                // 2. 히스토리 마이그레이션
                for (const h of localHistory) {
                    await saveAnalysisResult(h.file_name, h.analysis_data, 'guest_migration');
                }

                clearGuestData();
                toast.success("기존 작업 데이터를 계정으로 성공적으로 불러왔습니다! ✨", { id: 'migration' });
                loadHistory(); 
            } catch (err) {
                console.error("Migration failed", err);
                toast.error("데이터 통합 중 일부 오류가 발생했습니다.", { id: 'migration' });
            }
        };
    ...
    const handleMultiSheetsLoaded = (newSheets: Sheet[]) => {
        setSheets((prev: Sheet[]) => {
    ...
    const handleSearch = async (query: string) => {
        ...
        try {
            ...
            if (result.intent === 'generation' && result.generatedData) {
                ...
                const isPartialMerge = data.length > 0;

                if (isPartialMerge) {
                    // ── Smarter Patch/Merge Logic ──
                    // If AI returns same rows (by Count or ID), merge columns.
                    // If AI returns fewer rows, try to match by index or ID and update only those.

                    finalData = data.map((existingRow: any, idx: number) => {
                        // Look for matching row in AI result (either by _id or by index if count is same)
                        const aiMatchedRow = result.generatedData!.find((r: any, aiIdx: number) => 
                            (r._id && r._id === existingRow._id) || 
                            (result.generatedData!.length === data.length && aiIdx === idx)
                        );
                        
                        if (aiMatchedRow) {
                            // Merge only non-id fields from AI row into existing row
                            const { _id, ...cleanAiRow } = aiMatchedRow;
                            return { ...existingRow, ...cleanAiRow };
                        }
                        return existingRow;
                    });

                    // If AI returned MORE rows than existing (e.g., Appending), add them
                    if (result.generatedData!.length > data.length) {
                        const newRows = result.generatedData!.slice(data.length);
                        finalData = [...finalData, ...newRows];
                    }
                    
                    toast.success(`✨ 기존 데이터에 AI의 변경사항을 성공적으로 병합(Patch)했습니다.`);
                } else {
                    toast.success(`✨ ${result.generatedData.length}행의 데이터를 AI가 새로 생성했습니다.`);
                }
                ...
                // Auto saving: Cloud if logged in, Local if guest
    ...
    const handleGoogleSync = async () => {
        ...
        if (!providerToken) {
            toast.info('구글 시트 연동을 위해 구글 로그인이 필요합니다. 로그인 후 자동으로 다시 시도합니다.');
    ...
    return (
        ...
        >
            ...
            <Routes>
                <Route path="/" element={
                    <main className="flex-1 flex flex-col items-center pt-12 px-4 pb-20 space-y-8">
                        ...
                        <div className="w-full max-w-4xl space-y-8 text-center">
                            ...
                            <div className="relative group">
                                <SearchBar
                                    ...
                                    placeholder={hasData ? "이 표에서 '비고란 추가해줘'라고 말해 보세요!" : "무엇을 만들어 드릴까요? (예: 회사 비품 목록)"}
                                />
                                {/* Auto-Save Badge - Peace of Mind */}
                                <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 w-full flex items-center justify-center gap-2 text-[11px] text-gray-400 dark:text-gray-500 font-medium">
                                    <span className="flex h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse"></span>
                                    지금 하신 작업은 브라우저에 자동 저장됩니다 (로그인 불필요)
                                </div>
                            </div>
                        ...
                        {/* ── 온보딩 가이드 (데이터가 없을 때만 표시되는 '경험형 가이드') ── */}
                        {!hasData && (
                            <div className="w-full max-w-6xl mx-auto mt-16 space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                                <div className="text-center space-y-2">
                                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                                        EasyXL <span className="text-deepblue-600">3초 시작 가이드</span>
                                    </h3>
                                    <p className="text-sm text-gray-500 dark:text-gray-400 font-semibold tracking-tight">학습이 필요 없는 가장 직관적인 AI 엑셀 도구</p>
                                </div>
                                    ...
                                    {[
                                        {
                                            step: "01",
                                            title: "자연어로 데이터 생성",
                                            desc: "상단 제안 키워드를 누르거나 자유롭게 명령해보세요. 복잡한 표도 순식간에 완성됩니다.",
                                            icon: (
                                        ...
                                        },
                                        {
                                            step: "02",
                                            title: "말하듯이 자유로운 수정",
                                            desc: "필요한 열을 추가하거나 데이터를 필터링하고 싶을 땐, 그냥 말만 하세요. AI가 알아서 처리합니다.",
                                            icon: (
                                                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        },
                                        {
                                            step: "03",
                                            title: "스마트 데이터 다운로드",
                                            desc: "완성된 결과는 클릭 한 번으로 엑셀 또는 구글 시트로 내보내 실무에 즉시 활용하세요.",
                                            icon: (
                                                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div key={idx} className="relative p-8 bg-white dark:bg-[#1a1a1a] border border-gray-100 dark:border-gray-800 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                                            <div className={`w-12 h-12 ${item.color} rounded-2xl flex items-center justify-center mb-6`}>
                                                {item.icon}
                                            </div>
                                            <div className="absolute top-8 right-8 text-4xl font-black text-gray-50/50 dark:text-gray-900/10 pointer-events-none">
                                                {item.step}
                                            </div>
                                ...
                                <div className="flex flex-col items-center gap-4 py-8">
                                    <div className="flex items-center gap-4 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
                                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-1">Our Core Stack:</span>
                                        <div className="flex items-center gap-3">
                                            <span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded text-[10px] font-bold">Google Gemini AI</span>
                                            <span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded text-[10px] font-bold">Secure DB Syncing</span>
                                            <span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded text-[10px] font-bold">Handsontable Grid</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <h3 className="font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => renameSheet(activeSheetIndex, e.target.value)}
                                                    className="bg-transparent border-none outline-none focus:ring-2 focus:ring-indigo-500/30 rounded px-1 -ml-1 transition-all w-[240px] truncate"
                                                    placeholder="시트 이름 입력..."
                                                />
                            ...
                            <div className="w-full max-w-6xl mx-auto bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-xl animate-in fade-in zoom-in-95 duration-300 mb-10">
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                                        ...
                                        최근 분석 이력 { (history.length + guestHistory.length) > 0 && `(${history.length + guestHistory.length})` }
                                    </h3>
                                ...
                                { (history.length > 0 || guestHistory.length > 0) ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {/* Combine both lists */}
                                        {[...guestHistory, ...history].map(item => (
                                            <div
                                                key={item.id}
                                                className="flex items-center gap-2 p-3 rounded-xl bg-gray-50 dark:bg-[#262626] border border-transparent hover:border-blue-500/30 transition-all group"
                                            >
                                                <div 
                                                ...
                                                <button
                                                    onClick={async (e) => {
                                                        ...
                                                        if (item.user_id === 'guest') {
                                                             // Guest items are in store, but we can't easily delete individually via store yet,
                                                             // so let's just toast that it's local.
                                                             toast.error('로컬 데이터는 개별 삭제가 지원되지 않습니다. (로그인 후 통합 가능)');
                                                             return;
                                                        }
                                                        
                                                        // 1. 즉각적인 UI 반영 (Optimistic Update)
                                                        const originalHistory = [...history];
                                                        setHistory((prev: AnalysisHistoryRecord[]) => prev.filter(h => h.id !== item.id));
                                                        ...
                                                        toast.promise(
                                                            ...
                                                            {
                                                                loading: '삭제 요청 중...',
                                                                success: '분석 이력이 영구 삭제되었습니다.',
                                                                error: (err) => {
                                                                    setHistory(originalHistory);
                                                                    return `삭제 실패: ${err.message || '알 수 없는 오류'}`;
                                                                },
                                                            }
                                                        );
                                                    }}
                                                    className="p-3 text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/40 rounded-xl transition-all shadow-sm border border-transparent hover:border-red-200 dark:hover:border-red-900/50"
                                                    title="내역 영구 삭제"
                                                >
                                ...
                                {savedSheets.length > 0 ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {savedSheets.map(sheet => (
                                            <div key={sheet.id} className="flex items-center gap-2 p-3 rounded-xl bg-gray-50 dark:bg-[#262626] border border-transparent hover:border-purple-500/30 transition-all group">
                                                <div 
                                                ...
                                                <button
                                                    onClick={async (e) => {
                                                        ...
                                                        toast.promise(
                                                            ...
                                                            {
                                                                loading: '시트를 삭제하는 중...',
                                                                success: '시트가 영구 삭제되었습니다.',
                                                                error: (err) => `삭제 실패: ${err.message || '알 수 없는 오류'}`,
                                                            }
                                                        );
                                                    }}
                                                    className="p-3 text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/40 rounded-xl transition-all shadow-sm border border-transparent hover:border-red-200 dark:hover:border-red-900/50"
            ...
            {/* Simple Footer */}
            <footer className="w-full py-12 border-t border-gray-100 dark:border-gray-900">
                <div className="container mx-auto px-4">
                    <div className="flex flex-col items-center gap-6">
                        <div className="flex items-center gap-8 text-sm font-medium text-gray-500 dark:text-gray-400">
                            <Link to="/about" className="hover:text-deepblue-600 transition-colors">서비스 소개</Link>
                            <Link to="/guide" className="hover:text-deepblue-600 transition-colors">사용 가이드</Link>
                            <Link to="/support" className="hover:text-deepblue-600 transition-colors">고객 지원</Link>
                            <Link to="/privacy" className="hover:text-deepblue-600 transition-colors">개인정보처리방침</Link>
                        </div>
                        <p className="text-[11px] text-gray-400 dark:text-gray-600 tracking-wider uppercase">
