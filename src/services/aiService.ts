...
${selection?.selectedData?.length ? `- 선택 데이터: ${JSON.stringify(selection.selectedData.slice(0, 5))}` : ''}` : '- 현재 로드된 데이터가 없습니다. 사용자가 엑셀과 무관한 일반적인 질문을 한다면 intent를 chat으로 설정하여 답변하고, 데이터 생성을 원한다면 intent를 generation으로 설정하여 새로운 구조(열 이름들)를 정의하고 데이터를 생성하세요.'}`;
    ...
    // ── 상태 기반 프롬프트 가공 (기존 표 유지 전략) ──
    let stateAwarePrompt = '';
    if (fullData.length > 0) {
        stateAwarePrompt = `
[STATE CHECK: ACTIVE TABLE DATA EXISTS]
현재 그리드에 데이터가 존재합니다. 
AI에게 보내는 특별 지시: "현재 이런 표가 있으니, 이 구조를 유지하며 사용자 요청인 [${query}]을(를) 반영해서 '수정된 JSON(Patch/Update)'만 보내주세요. 절대로 전체를 새로 생성하지 마세요."
`;
    }
...
- [경고] generatedData의 키(Key)로 절대로 'A', 'B', 'C' 같은 알파벳을 쓰지 마세요. 무조건 '날짜', '항목', '금액' 등 의미 있는 한글/영어 이름을 사용하세요.
- 오직 JSON 형식으로만 응답하세요.`;
